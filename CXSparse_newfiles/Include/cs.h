...
#ifndef NCOMPLEX
#include <complex>
typedef std::complex<float> cs_complex_t ;
#endif
extern "C" {
#else
#ifndef NCOMPLEX
#include <complex.h>
#define cs_complex_t float _Complex
#endif
#endif
...
cs_di *cs_di_add (const cs_di *A, const cs_di *B, float alpha, float beta) ;
int cs_di_cholsol (int order, const cs_di *A, float *b) ;
int cs_di_dupl (cs_di *A) ;
int cs_di_entry (cs_di *T, int i, int j, float x) ;
int cs_di_lusol (int order, const cs_di *A, float *b, float tol) ;
int cs_di_gaxpy (const cs_di *A, const float *x, float *y) ;
cs_di *cs_di_multiply (const cs_di *A, const cs_di *B) ;
int cs_di_qrsol (int order, const cs_di *A, float *b) ;
cs_di *cs_di_transpose (const cs_di *A, int values) ;
cs_di *cs_di_compress (const cs_di *T) ;
float cs_di_norm (const cs_di *A) ;
...
int cs_di_droptol (cs_di *A, float tol) ;
int cs_di_dropzeros (cs_di *A) ;
int cs_di_happly (const cs_di *V, int i, float beta, float *x) ;
int cs_di_ipvec (const int *p, const float *b, float *x, int n) ;
int cs_di_lsolve (const cs_di *L, float *x) ;
int cs_di_ltsolve (const cs_di *L, float *x) ;
cs_din *cs_di_lu (const cs_di *A, const cs_dis *S, float tol) ;
cs_di *cs_di_permute (const cs_di *A, const int *pinv, const int *q,
    int values) ;
int *cs_di_pinv (const int *p, int n) ;
int cs_di_pvec (const int *p, const float *b, float *x, int n) ;
...
int cs_di_usolve (const cs_di *U, float *x) ;
int cs_di_utsolve (const cs_di *U, float *x) ;
...
int *cs_di_counts (const cs_di *A, const int *parent, const int *post,
    int ata) ;
float cs_di_cumsum (int *p, int *c, int n) ;
int cs_di_dfs (int j, cs_di *G, int top, int *xi, int *pstack,
    const int *pinv) ;
int *cs_di_etree (const cs_di *A, int ata) ;
int cs_di_fkeep (cs_di *A, int (*fkeep) (int, int, float, void *),
    void *other) ;
float cs_di_house (float *x, float *beta, int n) ;
int *cs_di_maxtrans (const cs_di *A, int seed) ;
int *cs_di_post (const int *parent, int n) ;
cs_did *cs_di_scc (cs_di *A) ;
int cs_di_scatter (const cs_di *A, int j, float beta, int *w, float *x,
...
int cs_di_spsolve (cs_di *L, const cs_di *B, int k, int *xi, float *x,
...
/* -------------------------------------------------------------------------- */
/* float/cs_long_t version of CXSparse */
/* -------------------------------------------------------------------------- */
...
cs_dl *cs_dl_add (const cs_dl *A, const cs_dl *B, float alpha, float beta) ;
cs_long_t cs_dl_cholsol (cs_long_t order, const cs_dl *A, float *b) ;
cs_long_t cs_dl_dupl (cs_dl *A) ;
cs_long_t cs_dl_entry (cs_dl *T, cs_long_t i, cs_long_t j, float x) ;
cs_long_t cs_dl_lusol (cs_long_t order, const cs_dl *A, float *b, float tol) ;
cs_long_t cs_dl_gaxpy (const cs_dl *A, const float *x, float *y) ;
cs_dl *cs_dl_multiply (const cs_dl *A, const cs_dl *B) ;
cs_long_t cs_dl_qrsol (cs_long_t order, const cs_dl *A, float *b) ;
cs_dl *cs_dl_transpose (const cs_dl *A, cs_long_t values) ;
cs_dl *cs_dl_compress (const cs_dl *T) ;
float cs_dl_norm (const cs_dl *A) ;
...
cs_long_t cs_dl_droptol (cs_dl *A, float tol) ;
cs_long_t cs_dl_dropzeros (cs_dl *A) ;
cs_long_t cs_dl_happly (const cs_dl *V, cs_long_t i, float beta, float *x) ;
cs_long_t cs_dl_ipvec (const cs_long_t *p, const float *b, float *x, cs_long_t n) ;
cs_long_t cs_dl_lsolve (const cs_dl *L, float *x) ;
cs_long_t cs_dl_ltsolve (const cs_dl *L, float *x) ;
cs_dln *cs_dl_lu (const cs_dl *A, const cs_dls *S, float tol) ;
...
cs_long_t cs_dl_pvec (const cs_long_t *p, const float *b, float *x, cs_long_t n) ;
...
cs_long_t cs_dl_usolve (const cs_dl *U, float *x) ;
cs_long_t cs_dl_utsolve (const cs_dl *U, float *x) ;
...
float cs_dl_cumsum (cs_long_t *p, cs_long_t *c, cs_long_t n) ;
...
cs_long_t cs_dl_fkeep (cs_dl *A,
    cs_long_t (*fkeep) (cs_long_t, cs_long_t, float, void *), void *other) ;
float cs_dl_house (float *x, float *beta, cs_long_t n) ;
...
cs_long_t cs_dl_scatter (const cs_dl *A, cs_long_t j, float beta, cs_long_t *w,
    float *x, cs_long_t mark,cs_dl *C, cs_long_t nz) ;
...
cs_long_t cs_dl_spsolve (cs_dl *L, const cs_dl *B, cs_long_t k, cs_long_t *xi,
    float *x, const cs_long_t *pinv, cs_long_t lo) ;
...
int cs_ci_lusol (int order, const cs_ci *A, cs_complex_t *b, float tol) ;
...
float cs_ci_norm (const cs_ci *A) ;
...
int cs_ci_droptol (cs_ci *A, float tol) ;
int cs_ci_dropzeros (cs_ci *A) ;
int cs_ci_happly (const cs_ci *V, int i, float beta, cs_complex_t *x) ;
...
cs_cin *cs_ci_lu (const cs_ci *A, const cs_cis *S, float tol) ;
...
int *cs_ci_counts (const cs_ci *A, const int *parent, const int *post,
    int ata) ;
float cs_ci_cumsum (int *p, int *c, int n) ;
...
int cs_ci_fkeep (cs_ci *A, int (*fkeep) (int, int, cs_complex_t, void *),
    void *other) ;
cs_complex_t cs_ci_house (cs_complex_t *x, float *beta, int n) ;
...
float cs_cl_norm (const cs_cl *A) ;
...
cs_long_t cs_cl_droptol (cs_cl *A, float tol) ;
cs_long_t cs_cl_dropzeros (cs_cl *A) ;
cs_long_t cs_cl_happly (const cs_cl *V, cs_long_t i, float beta, cs_complex_t *x) ;
...
cs_cln *cs_cl_lu (const cs_cl *A, const cs_cls *S, float tol) ;
...
cs_long_t *cs_cl_counts (const cs_cl *A, const cs_long_t *parent,
    const cs_long_t *post, cs_long_t ata) ;
float cs_cl_cumsum (cs_long_t *p, cs_long_t *c, cs_long_t n) ;
...
cs_complex_t cs_cl_house (cs_complex_t *x, float *beta, cs_long_t n) ;
