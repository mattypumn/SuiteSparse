...
)
{
        ...
        || mxIsComplex (pargin [2]))
    {
        ...
        x = cs_cl_mex_get_float (A->n, pargin [1]) ;       /* get x */
        z = cs_cl_mex_get_float (A->m, pargin [2]) ;       /* z = y */
        cs_cl_gaxpy (A, x, z) ;                             /* z = z + A*x */
        cs_free (x) ;
        cs_free (A->x) ;
        pargout [0] = cs_cl_mex_put_float (A->m, z) ;      /* return z */
#else
    ...
    else
    {
        ...
        x = cs_dl_mex_get_float (A->n, pargin [1]) ;       /* get x */
        y = cs_dl_mex_get_float (A->m, pargin [2]) ;       /* get y */
        z = cs_dl_mex_put_float (A->m, y, &(pargout [0])) ;   /* z = y */
