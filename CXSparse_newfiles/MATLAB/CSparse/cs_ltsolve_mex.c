...
)
{
    ...
    if (mxIsComplex (pargin [0]) || mxIsComplex (pargin [1]))
    {
        ...
        x = cs_cl_mex_get_float (L->n, pargin [1]) ;           /* x = b */
        cs_cl_ltsolve (L, x) ;                                  /* x = L'\x */
        cs_free (L->x) ;
        pargout [0] = cs_cl_mex_put_float (L->n, x) ;          /* return x */
#else
    ...
    else
    {
        ...
        b = cs_dl_mex_get_float (L->n, pargin [1]) ;           /* get b */
        x = cs_dl_mex_put_float (L->n, b, &(pargout [0])) ;    /* x = b */
