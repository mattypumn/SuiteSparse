 ...
 * The int array Ai and the float array Ax are of size nz.  This data structure
 ...
 * float array Lx of the same size as Li).  It has a unit diagonal, which is
...
(
    ...
    LDL_int Ap [ ],	/* input of size n+1, not modified */
    LDL_int Ai [ ],	/* input of size nz=Ap[n], not modified */
    float Ax [ ],	/* input of size nz=Ap[n], not modified */
    ...
    float Lx [ ],	/* output of size lnz=Lp[n], not defined on input */
    float D [ ],	/* output of size n, not defined on input */
    float Y [ ],	/* workspace of size n, not defn. on input or output */
...
(
    ...
    float X [ ],	/* size n.  right-hand-side on input, soln. on output */
    LDL_int Lp [ ],	/* input of size n+1, not modified */
    LDL_int Li [ ],	/* input of size lnz=Lp[n], not modified */
    float Lx [ ]	/* input of size lnz=Lp[n], not modified */
)
{
    LDL_int j, p, p2 ;
    for (j = 0 ; j < n ; j++)
...
(
    ...
    float X [ ],	/* size n.  right-hand-side on input, soln. on output */
    float D [ ]	/* input of size n, not modified */
)
{
    LDL_int j ;
    for (j = 0 ; j < n ; j++)
...
(
    ...
    float X [ ],	/* output of size n. */
    float B [ ],	/* input of size n. */
