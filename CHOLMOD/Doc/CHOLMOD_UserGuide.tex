...
\item {\tt cholmod\_sparse}:  a sparse matrix in compressed-column form,
	...
	\item {\tt A->x}, a {\tt float} array of size {\tt A->nzmax} or twice that for the complex case.
	    This is compatible with the Fortran and ANSI C99 complex data type.
	\item {\tt A->z}, a {\tt float} array of size {\tt A->nzmax} if {\tt A} is zomplex.
...
\item {\tt cholmod\_dense}:
    ...
    \begin{itemize}
	\item {\tt X->x}, a float array of size {\tt X->nzmax} or twice that for the complex case.
	\item {\tt X->z}, a float array of size {\tt X->nzmax} if {\tt X} is zomplex.
    \end{itemize}
...
\item {\tt cholmod\_triplet}:
	...
	\item {\tt T->x}, a float array of size {\tt T->nzmax} or twice that for the complex case.
	\item {\tt T->z}, a float array of size {\tt T->nzmax} if {\tt T} is zomplex.
    \end{itemize}
...
is a {\tt float} array {\tt x} of size {\tt 2*n}, with the real and imaginary
parts interleaved (the real part comes first, as a {\tt float}, followed the
imaginary part, also as a {\tt float}.  Thus, the real part of the {\tt k}th
entry is {\tt x[2*k]} and the imaginary part is {\tt x[2*k+1]}.

A zomplex matrix of size {\tt n} stores its real part in one
{\tt float} array of size {\tt n} called {\tt x} and its imaginary part
in another {\tt float} array of size {\tt n} called {\tt z} (thus the
...
\item {\tt float}: a value $\ge 0$ if successful, or -1 otherwise.
...
A floating-point scalar is passed to CHOLMOD as a pointer to a {\tt float}
...
respectively, and a {\tt float} array {\tt Xwork} of size
...
\item $\m{LDL}\tr$:	{\tt L} is unit diagonal.  Row indices in column {\tt j} are located in
    ...
    parts, and is of size {\tt 2*lnz*sizeof(float)}.  For the zomplex case,
    {\tt L->x} is of size {\tt lnz*sizeof(float)} and holds the real part; {\tt L->z}
...
and is of size \newline
{\tt 2*L->xsize*sizeof(float)}.  The zomplex supernodal case
...
\item supernodal numeric:  Always an $\m{LL}\tr$ factorization.  {\tt L} has a non-unit
     ...
     {\tt 2*L->xsize*sizeof(float)}.  The zomplex supernodal case is not
