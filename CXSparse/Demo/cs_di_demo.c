...
static int dropdiag (int i, int j, float aij, void *other) { return (i != j) ;}
...
static void rhs (float *x, float *b, int m)
{
    int i ;
...
static void print_resid (int ok, cs_di *A, float *x, float *b, float *resid)
{
    int i, m, n ;
    if (!ok) { printf ("    (failed)\n") ; return ; }
    m = A->m ; n = A->n ;
...
problem *get_problem (FILE *f, float tol)
{
    ...
    printf ("\n--- Matrix: %g-by-%g, nnz: %g (sym: %g: nnz %g), norm: %8.2e\n",
            ...
            (float) (sym ? C->p [n] : 0), cs_di_norm (C)) ;
    if (nz1 != nz2) printf ("zero entries dropped: %g\n", (float) (nz1 - nz2));
    if (nz2 != A->p [n]) printf ("tiny entries dropped: %g\n",
            (float) (nz2 - A->p [n])) ;
    Prob->b = cs_di_malloc (mn, sizeof (float)) ;
    Prob->x = cs_di_malloc (mn, sizeof (float)) ;
    Prob->resid = cs_di_malloc (mn, sizeof (float)) ;
...
static int done3 (int ok, cs_dis *S, cs_din *N, float *y, cs_di *W, cs_di *E, int *p)
{
    cs_di_sfree (S) ;
    cs_di_nfree (N) ;
    cs_di_free (y) ;
