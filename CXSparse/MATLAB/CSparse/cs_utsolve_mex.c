...
)
{
    ...
    if (mxIsComplex (pargin [0]) || mxIsComplex (pargin [1]))
    {
        ...
        x = cs_cl_mex_get_float (U->n, pargin [1]) ;           /* x = b */
        cs_cl_utsolve (U, x) ;                                  /* x = U'\x */
        cs_free (U->x) ;
        pargout [0] = cs_cl_mex_put_float (U->n, x) ;          /* return x */
#else
    ...
    else
    {
        ...
        b = cs_dl_mex_get_float (U->n, pargin [1]) ;           /* get b */
        x = cs_dl_mex_put_float (U->n, b, &(pargout [0])) ;    /* x = b */
